...
export default function MarzipanoViewer() {
  ...
  const SUPABASE_STORAGE_URL =
    "https://ejvnmawlifhdytktdzer.supabase.co/storage/v1/object/public/tours/project_01/";
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (savedConfig) {
        try {
          lastUsedConfigRef.current = JSON.parse(savedConfig);
        } catch (e) {}
      }
      const savedMenuPos = localStorage.getItem("360_last_menu_pos");
      if (savedMenuPos) {
        try {
          setMenuPos(JSON.parse(savedMenuPos));
        } catch (e) {}
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    projectData.scenes.forEach((sceneData) => {
      ...
      if (!hotspotElementsRef.current[sceneData.sceneId])
        hotspotElementsRef.current[sceneData.sceneId] = {};
      ...
      sceneData.hotspots.forEach((hotspot: any) => {
        ...
        hotspotVisual.style.setProperty(
          "--depth-scale",
          initialScale.toString(),
        );
        ...
        hotspotElementsRef.current[sceneData.sceneId][hotspot.targetSceneId] =
          hotspotVisual;

        const hs = currentSceneObj
          .hotspotContainer()
          .createHotspot(hotspotWrapper, {
            yaw: hotspot.yaw,
            pitch: hotspot.pitch,
          });
        ...
        if (isEditMode) {
            ...
            const onMouseUp = (upEvent: MouseEvent) => {
              ...
              if (isDragging) {
                ...
                if (coords) {
                  ...
                  console.log(
                    `📌 سهم الانتقال للمشهد [${hotspot.targetSceneId}] تم نقله!`,
                  );
                  ...
                  fetch("/api/update-hotspot", {
                    ...
                    .then((res) => {
                      if (!res.ok) {
                        console.warn(
                          "مسار الحفظ قيد التجهيز من قبل Turbopack...",
                        );
                    ...
                    .then((data) => {
                      if (data.success) {
                        console.log(
                          "✅ تم حفظ الإحداثيات تلقائياً في ملف JSON!",
                        );
                        hotspot.yaw = coords.yaw;
                        hotspot.pitch = coords.pitch;
                      } else
                        console.error("❌ حدث خطأ أثناء الحفظ:", data.error);
                    })
                    .catch((err) =>
                      console.error("❌ فشل الاتصال بالخادم للحفظ:", err),
                    );
                }
              } else {
                ...
                if (target) {
                  ...
                  if (
                    currentView &&
                    sceneData.northOffset !== undefined &&
                    target.data.northOffset !== undefined
                  ) {
                    const absoluteCompassYaw =
                      currentView.yaw() + sceneData.northOffset;
                    const newTargetYaw =
                      absoluteCompassYaw - target.data.northOffset;
          ...
          hotspotVisual.addEventListener("click", (e) => {
            ...
            if (target) {
              ...
              console.error(
                `المشهد المطلوب غير موجود في بيانات JSON: ${hotspot.targetSceneId}`,
              );
            }
          });
        }
    ...
    if (projectData.scenes.length > 0) {
      ...
      const sceneToLoad =
        currentSceneId && scenes[currentSceneId]
          ? currentSceneId
  ...
  const handleDeleteHotspot = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const scene = updatedData.scenes.find(
          (s: any) => s.sceneId === editingHotspot.sourceSceneId,
        );
        if (scene) {
          scene.hotspots = scene.hotspots.filter(
            (h: any) =>
              !(
                h.targetSceneId === editingHotspot.targetSceneId &&
                h.yaw === editingHotspot.yaw &&
                h.pitch === editingHotspot.pitch
              ),
          );
  ...
  const handleSaveEdit = async () => {
    ...
    const iconContainer =
      editingHotspot.domElement.querySelector(".hs-icon-container");
    if (iconContainer) {
      iconContainer.innerHTML = getHotspotStyle(
        editingHotspot.type,
        editingHotspot.customImageUrl,
        editingHotspot.opacity,
        editingHotspot.rotation,
      );
    }

    // تحديث الوجهة في الذاكرة فوراً لتجنب الحاجة لعمل ريفريش
    if (editingHotspot.newTargetSceneId) {
      editingHotspot.originalHotspotRef.targetSceneId =
        editingHotspot.newTargetSceneId;
    }
    ...
    localStorage.setItem(
      "360_last_hotspot_config",
      JSON.stringify(lastUsedConfigRef.current),
    );
    ...
    try {
      ...
      if (res.ok) {
        ...
        editingHotspot.originalHotspotRef.rotation =
          editingHotspot.rotation || 0;
      } else {
    ...
    if (coords) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const updatedData = JSON.parse(JSON.stringify(projectData));
          const sceneIndex = updatedData.scenes.findIndex(
            (s: any) => s.sceneId === currentSceneId,
          );
          if (sceneIndex !== -1) {
            updatedData.scenes[sceneIndex].hotspots.push({
              targetSceneId: targetId,
              yaw: coords.yaw,
              pitch: coords.pitch,
              type: lastUsedConfigRef.current.type,
  ...
  return (
    ...
    >
      ...
      {/* --- شريط الصور المصغرة (متاح للجميع مع إمكانية الطي) --- */}
      {projectData && (
        <>
          {/* زر الإظهار والإخفاء */}
          <button
            ...
            style={{
              position: "absolute",
              bottom: "20px",
              right: isGalleryOpen ? "150px" : "20px",
              zIndex: 1001,
          ...
          >
            {isGalleryOpen ? "إخفاء المشاهد ⏭" : "⏮ عرض المشاهد"}
          </button>
                ...
                <div
                  ...
                  onClick={() => {
                    if (!isActive && scenesRef.current[s.sceneId]) {
                      scenesRef.current[s.sceneId].scene.switchTo({
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      right: 0,
                      background: isActive
                        ? "linear-gradient(transparent, rgba(0, 255, 204, 0.3), rgba(0,0,0,0.9))"
                  ...
                  >
                    {editingSceneId === s.sceneId ? (
                      <input
                        ...
                        onBlur={() => handleSaveSceneName(s.sceneId)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter") handleSaveSceneName(s.sceneId);
                        }}
                        style={{
                          width: "90%",
                          background: "rgba(0,0,0,0.7)",
                        ...
                        {isEditMode && (
                          <span
                            onClick={(e) => {
                              e.stopPropagation();
                              setTempSceneName(
                                s.name || s.sceneId.split("_").pop(),
                              );
                              setEditingSceneId(s.sceneId);
                            }}
                            style={{
                              cursor: "pointer",
                              opacity: 0.7,
      ...
      {isEditMode && editingHotspot && projectData && (
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  الأشكال الأساسية المبنية في الكود:
                </label>
                <div
                  style={{
                    display: "grid",
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const iconContainer =
                          editingHotspot.domElement.querySelector(
                            ".hs-icon-container",
                          );
                        if (iconContainer)
                          iconContainer.innerHTML = getHotspotStyle(
              ...
              >
                ...
                >
                  الأيقونات المخصصة (من مجلد public/icons):
                </label>
                {availableIcons.length > 0 ? (
                    ...
                    {availableIcons.map((iconUrl) => (
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: "6px",
                          border:
                            editingHotspot.customImageUrl === iconUrl
                              ? "2px solid #0078ff"
                              : "2px solid transparent",
                          background: "#333",
                          padding: "4px",
                  ...
                  >
                    لم يتم العثور على أيقونات هنا.
                    <br />
                    قم بإنشاء مجلد باسم <b>icons</b> داخل مجلد <b>public</b>
                    <br />
                    وضع صورك بداخله لتظهر هنا تلقائياً للاختيار منها.
                  </div>
                )}
              </div>
                  ...
                  >
                    <div
                      style={{
                        ...
                        transform: `rotate(${editingHotspot.rotation || 0}deg) translate(-50%, -100%)`,
                        borderRadius: "2px",
                      }}
                    ></div>
                    <div
              ...
              >
                ...
                >
                  اسم الغرفة (يظهر عند الوقوف بالماوس):
                </label>
                <input
                  type="text"
          ...
          >
            <label
              style={{ fontSize: "14px", color: "#aaa", fontWeight: "bold" }}
            >
              وجهة السهم (الانتقال إلى مشهد):
            </label>
            <select
              value={
                editingHotspot.newTargetSceneId || editingHotspot.targetSceneId
              }
              onChange={(e) =>
                setEditingHotspot({
                  ...editingHotspot,
                  newTargetSceneId: e.target.value,
                })
              }
              style={{
                width: "100%",
